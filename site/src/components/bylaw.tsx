  ...
  return (
    <section id="why" className="scroll-mt-24 bg-ink text-bone">
      <div className="mx-auto grid max-w-[1160px] gap-12 px-5 py-bay sm:px-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-start lg:gap-20 lg:px-12">
        <div>
          <p className="cap mb-5 text-gold-light">{t.bylaw.eyebrow}</p>
          <h2 className="max-w-[12ch] text-h2 text-bone">{t.bylaw.heading}</h2>
          <div className="mt-8 space-y-6">
            {t.bylaw.paragraphs.map((p) => (
        ...
        <div>
          ...
          <dl className="grid">
            {t.bylaw.facts.map((f, i) => (
              <div
                key={f.value}
                className={i === 0 ? "pb-7" : "border-t border-gold/40 py-7 last:pb-0"}
              >
                <dt className="mb-2 font-display text-[clamp(2.25rem,4vw,3.25rem)] leading-none tabular-nums text-gold-light">
                  {f.value}
                </dt>
                <dd className="text-[0.875rem] leading-relaxed text-muted-dark">{f.label}</dd>
              </div>
            ))}
          </dl>
        </div>
